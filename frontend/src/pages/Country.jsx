...
export function Country() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await api.get('/api/weather/country', { params: { query: city } });
        if (mounted) setPayload(res.data);
      } catch (err) {
        if (mounted) setError(err?.response?.data?.message || err.message || 'Failed to load country data');
      } finally {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="text-xs text-slate-600 mt-1">Today summary for top cities in your selected country.</div>
