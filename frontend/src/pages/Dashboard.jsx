...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchWeatherFor(cityName) {
      if (!user) {
        const res = await api.get('/api/weather/public/default');
        return res.data;
      }
      const res = await api.get('/api/weather/city', { params: { query: cityName } });
      return res.data;
    }
    ...
    async function load() {
      ...
      try {
        ...
        const payload = await fetchWeatherFor('New York');
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const city = user ? getActiveCity() : 'New York';
        const res = user
          ? await api.get('/api/weather/city', { params: { query: city } })
          : await api.get('/api/weather/public/default');
        setData(res.data);
      } catch {
  ...
  async function onSearch(e) {
    ...
    if (!user) {
      localStorage.setItem(STORAGE_KEYS.pendingCity, trimmed);
      navigate('/login');
      return;
    }
    ...
    setError('');
    try {
      const res = await api.get('/api/weather/city', { params: { query: trimmed } });
      setData(res.data);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-slate-400">Mode: {modeLabel}</div>
          </div>
        ...
        <form onSubmit={onSearch} className="flex gap-2">
          ...
          />
          <button className="rounded-md bg-sky-600 hover:bg-sky-500 px-4 py-2 text-sm font-semibold">
            Search
          </button>
        </form>
        ...
        {!user ? (
          <div className="text-xs text-slate-400">
            Guest view shows New York by default. Searching a city will require login.
          </div>
        ) : null}
      </div>
    </div>
